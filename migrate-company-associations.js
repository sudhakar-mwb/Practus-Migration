...
/**
 * Migrates Company associations (Company<->Contact, Company<->Deal,
 * Company<->Company) from the source HubSpot portal to the destination
 * portal, using each record's tm_contact_record_id / tm_company_record_id /
 * tm_deal_record_id property to map source records to their already-migrated
 * destination counterparts.
 *
 * All the actual logic lives in ./hubspot-associations-common.js (shared by
 * this file and migrate-contact-associations.js / migrate-deal-associations.js)
 ...
 *
 * USAGE:
 *   node --env-file=.env migrate-company-associations.js
 *
...
if (require.main === module) {
  migratePrimaryObjectAssociations('companies').catch((err) => {
